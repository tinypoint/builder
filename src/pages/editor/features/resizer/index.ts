...
class Resizer {
  ...
  move = (e: MouseEvent) => {
    ...
    if ((code & DIR.LEFT || code & DIR.RIGHT)) {
      this._target.style.width = `${_width}px`;
    }
    if ((code & DIR.TOP || code & DIR.BOTTOM)) {
      this._target.style.height = `${_height}px`;
    }
    if ((code & DIR.LEFT)) {
  ...
  up = (e: MouseEvent) => {
    ...
    const {
      x, y, width, height, scale, baseScale, precision,
